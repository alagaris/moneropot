...
<template>
  <modal
    modal-id="entries"
    :title="accountId ? 'Your Entries' : 'All Entries (' + info.entries + ')'"
  >
    <span class="text-xs">
      SignKey
      ...
      <a
        target="_blank"
        :href="'https://xmrchain.net/block/' + info.sign_key"
        class="underline"
      >{{ info.sign_key }}</a>&nbsp;+&nbsp;
    ...
    <table class="table-auto w-full">
      ...
      <tbody>
        <tr v-for="(entry, index) in entries" :key="index">
          ...
          <td class="py-1 px-2">{{ entry.hash }}</td>
        </tr>
      </tbody>
    </table>
    <div class="text-center" v-if="hasMore">
      ...
      <button
        v-else
        type="button"
        class="bg-gray-500 px-3 py-2 text-white rounded-sm mt-4"
